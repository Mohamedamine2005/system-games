...
const type = require('../m3aksat.json');
const item = type[Math.floor(Math.random() * type.length)];
let author = msg.author;
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
msg.channel.send('**لديك 15 ثانية لكتابه المعاكسه**').then(async msg => {
const background = await Canvas.loadImage("https://media.discordapp.net/attachments/774373168856563738/800481634155364384/Web_Photo_Editor.jpg?width=864&height=225");
