...
const youtuber = require('../youtubers.json');
const item = youtuber[Math.floor(Math.random() * youtuber.length)];
let author = msgm.author;
const icon = await Canvas.loadImage(`${item.youtuber}`);
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
msgm.channel.send('**لديك 15 ثانية لمعرفه اسم القناه **').then(async msg => {
const background = await Canvas.loadImage('https://media.discordapp.net/attachments/758281926741065740/795070173425434644/Web_Photo_Editor_1.jpg');
...
ctx.fillStyle = '#ffffff';
      ...
      ctx.drawImage(icon, 100, 80, 170, 170);
...
await db.add(`points_${won.id}`, 3);
  ...
  msg.channel.send(`انتهاء الوقت ولم يقم احد بمعرفه من هو اليوتيوبر `);
})
})})}}