...
run: async (client, msgh, args, db , prefix) => {  
...
const type = require('../tarjma.json');
const item = type[Math.floor(Math.random() * type.length)];
let author = msgh.author;
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
msgh.channel.send('**لديك 15 ثانية لكتابة الترجمة**').then(async msg => {
const background = await Canvas.loadImage("https://media.discordapp.net/attachments/758281926741065740/779106324423770142/Web_Photo_Editor_9.jpg");
...
await db.add(`points_${won.id}`, 3);
  ...
  msg.channel.send(`انتهاء الوقت ولم يقم احد بــترجمة الجملة`);
})
})})}}