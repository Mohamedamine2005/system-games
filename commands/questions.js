...
const type = require('../quest.json');
const item = type[Math.floor(Math.random() * type.length)];
let author = message.author;
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
message.channel.send('**لديك __15__ ثانيه لتحل السوأل**').then(async msg => {
const background = await Canvas.loadImage("https://media.discordapp.net/attachments/732699537168138340/778753213922541579/Web_Photo_Editor_4.jpg");
...
msg.channel.send(`انتهاء الوقت ولم يقم احد بـحل السؤال`);
})})}
)
}
}
