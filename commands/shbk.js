...
run: async (client, msgj, args, db , prefix) => {  
...
const type = require('../shabk.json');
const item = type[Math.floor(Math.random() * type.length)];
let author = msgj.author;
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
msgj.channel.send('**لديك 15 ثانية لتشبيك الجمله**').then(async msg => {
const background = await Canvas.loadImage("https://media.discordapp.net/attachments/758281926741065740/779106323425132544/Web_Photo_Editor_8.jpg");
...
await db.add(`points_${won.id}`, 3);
  ...
  msg.channel.send(`انتهاء الوقت ولم يقم احد بـتشبيك الجملة`);
})
})})}}