...
const type = require('../who.json');
const item = type[Math.floor(Math.random() * type.length)];
let author = msg.author;
const filter = response => {
return item.answers.some(answer => answer.toLowerCase() === response.content.toLowerCase());};
msg.channel.send('**لديك 15 ثانية لكتابة الجملة**').then(async msg => {
const background = await Canvas.loadImage("https://media.discordapp.net/attachments/758281926741065740/779106466840969237/Web_Photo_Editor_3.jpg");
...
ctx.fillText(item.type  , 115, 130);
